...
4. **File mode only**: a tonic (Sa) frequency in Hz, or Enter to auto-detect from the opening ~5 seconds of audio (`FeatureExtractor.auto_detect_tonic()`, median method), which is treated as a calibration reference and excluded from the classification histogram.
