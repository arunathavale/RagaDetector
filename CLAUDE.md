...
Live mic mode runs for a fixed 5-minute window (`SESSION_DURATION_SECONDS` in `main_22.py` — a shorter window risked landing mostly in the alap before a performance settles into a fuller range of characteristic phrases), redraws a terminal dashboard once per second, and prints a JSON summary on exit. File mode processes the whole file once and prints a one-shot summary plus the same JSON block — no artificial time cap, since a file already has a finite, known length.
