...
- **Phase 1** (`LIVE_TONIC_WINDOW_SECONDS`, 2 min): listens silently — no dashboard — while the singer performs normally, accumulating raw detected pitches. At the end, `determine_tonic_from_pitches()` finds the tonic via **drone-interval priors** — a tanpura almost always drones on Sa+Pa (fifth) or Sa+Ma (fourth), a raga-agnostic structural signal from how the accompanying instrument is tuned, not from what's being sung or which raga it turns out to be. Same technique as `find_optimal_tonic.py`'s `score_drone_priors()`/`find_best_tonic_by_drone_priors()`, reimplemented locally to avoid a circular import. `print_tonic_stability_check()` then splits Phase 1's pitches into 4 time-ordered segments and re-runs the same drone-prior search on each, printing the spread — tonic essentially never changes mid-performance, so disagreement across segments flags a pitch-tracking problem rather than a real shift (same diagnostic as `find_optimal_tonic.py`'s `check_tonic_stability()`).
